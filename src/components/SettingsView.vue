				...
				<template v-if="tdeeResult !== null">
					...
					<span class="settings-tdee-result__breakdown">
						{{ selectedGoal.adjustment > 0
							? t('calorietracker', 'BMR {bmr} {unit} × {factor} activity + {energy} {unit} adjustment', { bmr: displayEnergy(tdeeBMR), unit: energyLabel, factor: selectedActivity.factor, energy: displayEnergy(Math.abs(selectedGoal.adjustment)) })
							: selectedGoal.adjustment < 0
								? t('calorietracker', 'BMR {bmr} {unit} × {factor} activity − {energy} {unit} adjustment', { bmr: displayEnergy(tdeeBMR), unit: energyLabel, factor: selectedActivity.factor, energy: displayEnergy(Math.abs(selectedGoal.adjustment)) })
								: t('calorietracker', 'BMR {bmr} {unit} × {factor} activity', { bmr: displayEnergy(tdeeBMR), unit: energyLabel, factor: selectedActivity.factor }) }}
					</span>
				</template>
			</div>
